<template>
  ...
  >
    ...
    <div class="mb-4">
      <Wysisyg @update="updateForm" :content="article.editor" />
      <Field
        name="editor"
        v-model="veditor"
        v-slot="{ field, errors, errorMessage }"
    ...
    <v-btn type="submit" variant="outlined"> Edit Article </v-btn>
  </Form>
</template>
<script setup>
import { ref } from "vue";
...
async function onSubmit(values, { resetForm }) {
  loading.value = true;
  await useArticle.updateArticle(route.params.id, values).finally(() => {
    loading.value = false;
  });
}
...
const article = ref({});
//get article by id and popualte
useArticle
  .getArticleById(route.params.id)
  .then((res) => {
    article.value = { ...res };
    updateForm(res.editor);
    loading.value = false;
  })
